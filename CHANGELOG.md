...
- Gate inspector "Emit signal" button: each signal-kind wait term in the gate inspector now shows an **Emit signal** button (visible while the wait is pending). Clicking reveals an inline form with the signal key prefilled (read-only) and a JSON payload textarea defaulting to `{ "approved": true }`. Submitting calls `POST /api/pro/workflows/{id}/task-signals`, invalidates the workflow query, and shows a success toast. Invalid JSON is caught client-side and blocks submission. [PR #TBD](https://github.com/riverqueue/riverui/pull/TBD).
...
- Upgrade River to v0.31.0 and River Pro to v0.22.0. [PR #511](https://github.com/riverqueue/riverui/pull/511).

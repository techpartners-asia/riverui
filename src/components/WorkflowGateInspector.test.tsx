...
describe("WorkflowGateInspector", () => {
  ...
  it("truncates long CEL terms until expanded", async () => {
    const longCel =
      'payload.approved == true && payload.reviewer != "" && payload.review_score >= 90 && payload.region in ["us", "ca"]';
    const wait: WorkflowTaskWait = {
      exprCel: "manual_approval_received",
      inputs: {
        deps: [],
        signals: [],
        timers: [],
      ...
      terms: [
        {
          ...
          name: "manual_approval_received",
          result: { matchedCount: 0, requiredCount: 1, satisfied: false },
          signalKey: "manual.approval",
        },
      ],
    };
    ...
    expect(screen.queryByText(longCel)).not.toBeInTheDocument();
    expect(
      screen.getByText(
    ...
    await act(async () => {
      fireEvent.click(
        screen.getByRole("button", {
          name: "Show full CEL expression for Manual approval",
        }),
      );
    });

    expect(screen.getByText(longCel)).toBeInTheDocument();
    expect(
      screen.getByRole("button", {
        name: "Hide full CEL expression for Manual approval",
      }),
    ).toHaveAttribute("aria-expanded", "true");
  });

  it("keeps signal evidence controls visible when long CEL expands", async () => {
    const longCel =
      'payload.action == "approved" && payload.reviewer.email.endsWith("@example.com") && payload.source == "console"';
    const wait: WorkflowTaskWait = {
    ...
    expect(
      screen.getByRole("button", { name: /Signal history/ }),
    ).toBeInTheDocument();

    await act(async () => {
      fireEvent.click(
        screen.getByRole("button", {
          name: "Show full CEL expression for Approval received",
        }),
      );
    });
  ...
  it("renders active wait evidence from diagnostics", async () => {
    ...
    const wait: WorkflowTaskWait = {
      exprCel: "approval_received || review_timeout",
      inputs: {
        deps: [],
        signals: [{ key: "approval.received" }],
        timers: [
    ...
    renderInspector(wait, {
      loadTaskWaitDiagnostics: async () => ({
        ...
        inputs: {
          ...
          timers: [
            {
              fireAt: new Date("2026-04-21T17:55:00Z"),
              fired: true,
              name: "review_timeout",
            },
          ],
    ...
    expect(
      await screen.findByText("2 of 2 conditions satisfied"),
    ).toBeInTheDocument();
    expect(screen.getByText("Included")).toBeInTheDocument();
